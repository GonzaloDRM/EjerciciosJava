...
public class Servicios {
    ...
    public static void Rellenar(Arreglos arrays){        
        for (int i=0;i<50;i++){
            arrays.getArray1()[i]=(double) Math.round(Math.random()*10000)/100;
        }
        for (int i=0;i<20;i++){
            arrays.getArray2()[i]=(double) Math.round(Math.random()*10000)/100;
        }
    }
        ...
        for (int i=0;i<50;i++){
            for (int k=1;k<50;k++){
                if (arrays.getArray1()[k]<arrays.getArray1()[k-1]){
                    double guardar = arrays.getArray1()[k-1];
